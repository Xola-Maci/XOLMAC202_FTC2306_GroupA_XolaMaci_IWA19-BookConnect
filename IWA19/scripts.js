...
dataList.addEventListener('click', function(event){
    ...
    bookSubtitle.innerHTML = `${authors[showingBook.author]} (${(showingBook.published).slice(0, 4)})` 
...
btnList.innerHTML = `<span>Show more</span>
    <span class="list__remaining">${Math.max(0, books.length - (0 * BOOKS_PER_PAGE))}</span>`;
...
const searchBooks = (event) => {
    ...
    for (const book of books) {
      const bookTitleMatch = filters.title.trim() === '' || book.title.toLowerCase().includes(filters.title.toLowerCase());
